    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
        {options.map((option) => (
          ...
          >
            <div
              className={`mb-6 p-4 rounded-full ${selectedLevel === option.id ? 'bg-red-600 text-white' : 'bg-gray-800 text-gray-400 group-hover:text-white group-hover:bg-gray-700'}`}
            >
              {option.icon}
            </div>
            <h3
              className={`text-xl font-bold mb-2 ${selectedLevel === option.id ? 'text-white' : 'text-gray-200'}`}
            >
              {option.title}
            </h3>
