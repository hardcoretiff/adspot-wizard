...
const ProcessingOverlay: React.FC<ProcessingOverlayProps> = ({ isOpen, steps }) => {
  ...
  const getIcon = (id: string) => {
    switch (id) {
      ...
      case 'subaccount': return <Server size={18} />;
      case 'assets': return <Shield size={18} />;
      case 'contact': return <User size={18} />;
      case 'campaign': return <Database size={18} />;
      case 'finalize': return <Rocket size={18} />;
    ...
    <div className="fixed inset-0 bg-black/90 z-[60] flex items-center justify-center backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-adspot-darkGray border border-gray-800 rounded-2xl w-full max-w-md p-8 shadow-2xl shadow-black relative overflow-hidden">
          ...
          {steps.map((step, index) => {
            ...
            return (
              ...
              >
                ...
                `}>
                  {isCompleted ? <Check size={16} strokeWidth={3} /> : isLoading ? <Loader2 size={16} className="animate-spin" /> : getIcon(step.id)}
                </div>
                
                <span className={`font-medium text-sm transition-colors ${isCompleted ? 'text-green-100' : isLoading ? 'text-white' : 'text-gray-500'}`}>
                  {step.label}
                </span>

                {isCompleted && (
                  <span className="ml-auto text-xs font-bold text-green-500 animate-in zoom-in duration-300">DONE</span>
                )}
              </div>
            );
          })}
        ...
        <div className="mt-8 text-center">
            <p className="text-xs text-gray-600 font-mono">Secure connection established with GoHighLevel API</p>
        </div>
      </div>
    </div>
  );
